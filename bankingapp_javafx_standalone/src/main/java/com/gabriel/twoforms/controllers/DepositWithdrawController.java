...
public class DepositWithdrawController {
    ...
    private void processTransaction(boolean isDeposit) {
        ...
        try {
            ...
            String actionTitle = isDeposit ? "Confirm Deposit" : "Confirm Withdrawal";
            String header = isDeposit 
                ? "Are you sure you want to deposit $" + String.format("%,.2f", amount) + "?" 
                : "Are you sure you want to withdraw $" + String.format("%,.2f", amount) + "?";
            
            double expectedBalance = isDeposit ? acc.getBalance() + amount : acc.getBalance() - amount;
            ...
            boolean confirmed = DialogUtil.confirm(
                ...
                header,
                body,
                isDeposit ? "Yes, Deposit" : "Yes, Withdraw"
            );
            if (!confirmed) return;
            boolean success;
            if (isDeposit) {
                success = BankingService.getInstance().deposit(acc.getAccountNumber(), amount, "Deposit");
            } else {
                success = BankingService.getInstance().withdraw(acc.getAccountNumber(), amount, "Withdrawal");
            }

            if (success) {
                showSuccess((isDeposit ? "Deposit" : "Withdrawal") + " successful!");
                amountField.clear();
                ...
                showError(isDeposit ? "Deposit failed." : "Withdrawal failed. Insufficient funds.");
            }
